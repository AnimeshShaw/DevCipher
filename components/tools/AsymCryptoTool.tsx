...
import { Copy, Check, RefreshCw, Key, Lock, Unlock, PenLine, ShieldCheck, AlertCircle, CheckCircle2, Keyboard } from 'lucide-react'
...
export default function AsymCryptoTool({ toolId }: Props) {
  ...
  const variant = toolId.split('-').slice(1).join('-')
    ...
    try {
      if (isECDSA) {
        ...
        if (variant === 'keygen') {
          ...
          if (!privateKey || !message) throw new Error('Private key and message required')
          setSignature(rsaSign(message, privateKey, rsaHashAlgo))
        } else if (variant === 'verify') {
          if (!publicKey || !message || !signature) throw new Error('Public key, message, and signature required')
          setVerifyResult(rsaVerify(message, signature, publicKey, rsaHashAlgo))
        } else if (variant === 'encrypt') {
          if (!publicKey || !message) throw new Error('Public key and message required')
          setOutput(rsaEncrypt(message, publicKey, rsaEncScheme))
        } else if (variant === 'decrypt') {
          if (!privateKey || !message) throw new Error('Private key and ciphertext required')
          setOutput(rsaDecrypt(message, privateKey, rsaEncScheme))
        }
      }
    } catch (e) {
  ...
  }, [privateKey, publicKey, message, signature, rsaBits, rsaHashAlgo, rsaEncScheme, ecCurve])

  const actionLabel = {
    'keygen': 'Generate Key Pair', 'sign': 'Sign Message',
    'verify': 'Verify Signature', 'encrypt': 'Encrypt', 'decrypt': 'Decrypt',
  }[variant] ?? 'Execute'
        ...
        <div className="space-y-3">
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4 space-y-4">
            ...
            {isRSA && (variant === 'sign' || variant === 'verify') && (
              <div className="space-y-1.5">
                <label className="text-xs text-zinc-400">Hash Algorithm</label>
                <Select value={rsaHashAlgo} onValueChange={setRsaHashAlgo}>
                  <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {['sha1', 'sha256', 'sha384', 'sha512'].map((h) => (
                      <SelectItem key={h} value={h} className="text-xs">{h.toUpperCase()}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            ...
            {isRSA && (variant === 'encrypt' || variant === 'decrypt') && (
              <div className="space-y-1.5">
                <label className="text-xs text-zinc-400">Encryption Scheme</label>
            ...
            <div className="pt-2 border-t border-zinc-800 text-[11px] text-zinc-600 space-y-1">
              <p>All operations run locally in your browser. No data is transmitted.</p>
            </div>
          </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {(variant === 'keygen' || variant === 'sign' || variant === 'decrypt' || variant === 'verify') && (
              <FieldBox label="Private Key" value={privateKey}
                onChange={variant !== 'keygen' ? setPrivateKey : undefined}
                readOnly={variant === 'keygen'}
                placeholder="Paste private key (PEM or hex)..." rows={6} />
            )}
            {(variant === 'keygen' || variant === 'verify' || variant === 'encrypt' || variant === 'sign') && (
              <FieldBox label="Public Key" value={publicKey}
                onChange={variant !== 'keygen' ? setPublicKey : undefined}
                readOnly={variant === 'keygen'}
                placeholder="Paste public key (PEM or hex)..." rows={6} />
            )}
          </div>
          ...
          {(variant === 'sign' || variant === 'verify') && (
            <FieldBox label="Signature (Hex/Base64)" value={signature}
              onChange={variant === 'verify' ? setSignature : undefined}
              readOnly={variant === 'sign'}
              placeholder="Signature will appear here..." rows={2} />
          )}
